#!/usr/bin/env python3
"""MCP-driven single-hypothesis report runner for the CIRBP liver workflow.
...
evidence-grading tables for one hypothesis (here: liver Cirbp cell-of-origin).
...
results/), this driver needs the bundled MCP engine and the atlas h5ad:
  ...
  - Atlas data + pre-extracted liver h5ad: ``--aging-atlas-home`` or env
    ``AGING_ATLAS_HOME``.

The hypothesis spec is built directly from explicit parameters (no auto-parser).
Parameters default to the liver Cirbp hypothesis but can be overridden on the
command line.
...
def _default_mcp_home() -> str:
    # HERE is scripts/; scripts/ -> cirbp_liver_single_cell/ -> analysis/ -> repo_root
    repo_root = HERE.parents[2]  # parents: [scripts, cirbp_..., analysis] -> repo_root
...
def main():
    ...
    ap.add_argument("--hyp-id", default="cirbp_liver_single_cell",
    ...
    ap.add_argument("--aging-atlas-home", default=os.environ.get("AGING_ATLAS_HOME"),
                    help="Path to the Mouse Aging Atlas data + pre-extracted "
                         "liver h5ads (or set AGING_ATLAS_HOME).")
    ...
    ap.add_argument("--h5ad", default="liver_hepatocyte_myeloid_comparison.h5ad",
                    help="Atlas-derived h5ad filename (relative to "
                         "--aging-atlas-home) or absolute path.")
    ap.add_argument("--tissue", default="Liver")
    ap.add_argument("--gene", default="Cirbp")
    ap.add_argument("--candidate-cell-types", default="Hepatocytes, Myeloid cells")
    ...
    ap.add_argument("--out", default=str(HERE.parent / "results" / "mcp"))
    a = ap.parse_args()
